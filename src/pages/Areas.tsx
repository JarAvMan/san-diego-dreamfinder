...
const Areas = () => {
  ...
  const getNeighborhoodsByRegion = (regionName: string) => {
    ...
    return sandiegoNeighborhoods.filter(hood => regionNames.includes(hood.name));
  };
  ...
  const paginatedNeighborhoods = displayedNeighborhoods.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
  ...
  return <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-1 container py-12">
        ...
        <div className="flex flex-wrap gap-3 mb-8">
          <button className={`px-4 py-2 rounded-full text-sm font-medium ${!selectedRegion ? 'bg-primary text-primary-foreground' : 'bg-muted hover:bg-muted/80'}`} onClick={() => setSelectedRegion(null)}>
            All Areas ({sandiegoNeighborhoods.length})
          </button>
          <button className={`px-4 py-2 rounded-full text-sm font-medium ${selectedRegion === 'coastal' ? 'bg-primary text-primary-foreground' : 'bg-muted hover:bg-muted/80'}`} onClick={() => setSelectedRegion('coastal')}>
            Coastal Communities ({getNeighborhoodsByRegion('coastal').length})
          </button>
          <button className={`px-4 py-2 rounded-full text-sm font-medium ${selectedRegion === 'central' ? 'bg-primary text-primary-foreground' : 'bg-muted hover:bg-muted/80'}`} onClick={() => setSelectedRegion('central')}>
            Central San Diego ({getNeighborhoodsByRegion('central').length})
          </button>
          <button className={`px-4 py-2 rounded-full text-sm font-medium ${selectedRegion === 'north' ? 'bg-primary text-primary-foreground' : 'bg-muted hover:bg-muted/80'}`} onClick={() => setSelectedRegion('north')}>
            North County ({getNeighborhoodsByRegion('north').length})
          </button>
          <button className={`px-4 py-2 rounded-full text-sm font-medium ${selectedRegion === 'east' ? 'bg-primary text-primary-foreground' : 'bg-muted hover:bg-muted/80'}`} onClick={() => setSelectedRegion('east')}>
            East County ({getNeighborhoodsByRegion('east').length})
          </button>
          <button className={`px-4 py-2 rounded-full text-sm font-medium ${selectedRegion === 'south' ? 'bg-primary text-primary-foreground' : 'bg-muted hover:bg-muted/80'}`} onClick={() => setSelectedRegion('south')}>
        ...
        <div className="space-y-8">
          <section>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {paginatedNeighborhoods.map(hood => <div key={hood.id} className="border rounded-lg overflow-hidden shadow-sm bg-card hover:shadow-md transition-shadow">
                  <div className="aspect-[16/9] overflow-hidden">
                    <img src={hood.image} alt={hood.name} className="w-full h-full object-cover transition-transform hover:scale-105" />
                  </div>
                  <div className="p-4">
                    <h3 className="font-medium text-lg mb-1">{hood.name}</h3>
                    <div className="flex gap-2 flex-wrap mb-2">
                      {hood.tags.slice(0, 3).map(tag => <span key={tag} className="px-2 py-1 bg-primary/10 text-primary rounded-full text-xs">
                          {tag}
                        </span>)}
                    </div>
                    ...
                    <a href={hood.kvCoreLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border border-input bg-background hover:bg-accent hover:text-accent-foreground h-9 rounded-md px-3">
                      View Properties
                    </a>
                  </div>
                </div>)}
